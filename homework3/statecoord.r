...
D = data.frame(D)
choices = c("Income","Illiteracy","Murder","HSGrad","Frost")
D[["LifeExpectancy"]]=cut(D[,"LifeExp"],quantile(D[,"LifeExp"]),c("Short","Below Average","Above Average","Long"),include.lowest=T)
...
p <- ggparcoord(data = D, 
                ...
                #columns = 1:4, 
                columns = sapply(c("Income","Illiteracy","Murder","HSGrad","Frost"),function(name){which(name==colnames(D))}),
                
                # Which column to use for coloring data
                #groupColumn = 5, 
                groupColumn = which("LifeExpectancy"==colnames(D)),
...
#lab_z <- c(sapply(D[choices], min), sapply(D[choices], max))
